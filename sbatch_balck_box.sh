...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --output=AbsUnBlack410mIdx1.log
#SBATCH --error=AbsUnaBlack410mIdx1.err
#SBATCH --mem=150G
...
#srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 bash -c "$(declare -f srun_parallel); srun_parallel absolute 410m untruncated 0 2 8" &
#srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 bash -c "$(declare -f srun_parallel); srun_parallel absolute 410m untruncated 0 2 8" &
#srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 bash -c "$(declare -f srun_parallel); srun_parallel absolute 410m  untruncated 0 2 8" &
...
srun --ntasks=1 --gres=gpu:1 bash -c "$(declare -f srun_parallel); srun_parallel absolute 410m untruncated 0 1 4" &
# 等待所有任务结束
wait