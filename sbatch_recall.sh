...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --output=recall2.8brelatr.log
#SBATCH --error=recall2.8brelatr.err
...
# 启动并行任务
srun --ntasks=1 --cpus-per-task=8 --gres=gpu:1 bash -c "$(declare -f srun_parallel); srun_parallel relative 70m truncated 0 0 2" &
