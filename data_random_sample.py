        ...
        for i in range(enumerate_length):
            ...
            if args.relative_length:
                ...
                if args.select_method == "truncate":
                    os.makedirs(f"./absolute_filtered_dataset_{idx+1}/{min_length}_{max_length}_truncated/{dataset_name}", exist_ok=True)
                    dataset.save_to_disk(f"./absolute_filtered_dataset_{idx+1}/{min_length}_{max_length}_truncated/{dataset_name}")
                else:
                    os.makedirs(f"./absolute_filtered_dataset_{idx+1}/{min_length}_{max_length}_nontruncated/{dataset_name}", exist_ok=True)
                    dataset.save_to_disk(f"./absolute_filtered_dataset_{idx+1}/{min_length}_{max_length}_nontruncated/{dataset_name}")
            print(dataset)