...
def figure_draw(data_dict, title,dataset_name, args):
    ...
    for ax, (dataset_name, dataset_loss) in zip(axs.flatten(), data_dict.items()):
        ...
        ax.legend()
    plt.tight_layout()
    os.makedirs(f"{args.save_dir}_figures_{args.dataset_idx}/{dataset_name}/{args.relative}/{args.truncated}/{args.min_len}_{args.min_len+100}", exist_ok=True)
    if args.same_length == True:
        plt.savefig(f"{args.save_dir}_figures_{args.dataset_idx}/{dataset_name}/{args.relative}/{args.truncated}/{args.min_len}_{args.min_len+100}/{title}_histograms_{args.model_size}_{dataset_name}_same_len.png")
    else:
        plt.savefig(f"{args.save_dir}_figures_{args.dataset_idx}/{dataset_name}/{args.relative}/{args.truncated}/{args.min_len}_{args.min_len+100}/{title}_histograms_{args.model_size}_{dataset_name}_all_len.png")
    plt.show()
...
def recall_collection(model, tokenizer, dataset, dataset_name, prefix, args, min_len=50, upper_limit=10000):
    ...
    for idx, (data_batch, orig_indices_batch) in tqdm(
        ...
        cond_outputs, cond_tokenized_inputs, cond_target_labels = caculate_outputs(model, tokenizer, prefix_batched_text, args, device=args.cuda,
                                                                                   min_len=min_len)
